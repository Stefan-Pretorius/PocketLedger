...
import {
  ...
  calculateSalarySacrificeBenefit, getTaxYearRates, getMarginalRate,
  calculateSaStampDuty,
} from "../tax";
import { runMonteCarlo, normalRandom, type McHoldingInput, type McPoint, type McHoldingLine } from "../monte-carlo";
...
function useScenarioProjection(scenario: ScenarioConfig, holdings: Holding[], summaries: { holding: Holding; marketValue: number }[]) {
  ...
  return useMemo(() => {
    ...
    const ownerRetireAge = scenario.owner === "partner" ? partnerRetirementAge : scenario.owner === "joint"
      ? Math.min(retirementAge ?? Infinity, partnerRetirementAge ?? Infinity)
      : retirementAge;
