...
- **Goal-as-envelope budget model**: `getBudgetSummary` now separates `allocatedToGoals` from category allocation — goal contributions count toward "Ready to Assign" spending but are excluded from Dashboard/Trends/print spending; Budget main view, CategoryCard, BudgetPrintModal, Dashboard StatCard, Trends, and Goal cards all show goal funding
- **Duplicate import gate**: `previewImport` tags already-imported rows (same date, ±0.005 amount, normalized description) with `isDuplicate`; Statements review shows a warning banner with "Skip duplicates" / "Reimport anyway" and disables Import until resolved
- **Planner defaults + UI consistency**: `usePlannerInputs` in FinancialPlanner re-evaluates Settings defaults live; RentVsBuy ages and FIRE retirement age default from Settings (`selfAge`/`selfRetirementAge`) instead of hardcoded values. Shared `SearchInput` (icon + clear button) and `FilterPill` components in `ui.tsx`; view-mode segmented toggles standardized; Expenses/Budget/Investments/Settings/Statements search+filter unified; MoneyFlow Verify, ScenarioBar Save/Load/Delete, FinancialPlanner Apply, and Not Found page use shared components
- **AI statement matching (local opencode server)**: `aimatch.ts` talks to a standalone `opencode serve` on port 4096 (`POST /session`, `POST /session/:id/message`, `GET /global/health`, Basic auth `opencode:<password>`). `getEffectiveConfig()` falls back to the default local URL so AI features work with zero config once the server is running; fetch/401/HTML-page errors are actionable (HTML usually means the Server URL points at the app itself or another web service, not opencode). `useAiChat` drops the cached session id on send failure (server restarts). Settings command binds `127.0.0.1` when served from localhost (no password when none set) vs `0.0.0.0` for LAN/phone. Verified live against opencode CLI 1.18.18 (health, session, message, CORS, Basic auth all correct)
- **Rent vs Buy transparency + fixes**: New "How It's Calculated (Year 1)" card shows purchase price, deposit, SA stamp duty, total upfront (renter invests this), year-1 renter/buyer yearly savings (income − housing only, other living costs excluded equally). Fixed double-counting of rent in `totalRentPaid` when downsizing to "rent"; fixed `rentPerYear` display being one year ahead. Savings look large because ALL income after housing is assumed invested over up to 60 years at the nominal return rate
